...
class ProfilePediatricianMedicalReport extends React.Component {
    ...
    componentDidMount() {
        ...
        axios.get(`/api/pediatrician/getPatientReports/${OIB}`).then(res => {
                ...
                <div className={"bigboy_left"}>
                    {medRep && Array.isArray(medRep) ? (
                        medRep.map((report) => (
                            <div key={report.reportId} className={"patient_list_instance"}>
                                <div className={"patient_list_infobox"}>
                                    <div className={"patient_list_ime_prezime ped_text_color"}>{`${patient.lastNameChild} ${patient.nameChild}`}</div>
                                    <div className={"patient_list_oib"}>{new Date(report.dateOfReport).toLocaleDateString()}</div>
                                </div>
                                <div className={"patient_list_buttonbox "}>
                                    <Link to={`/pediatrician/patientprofile/${patient.oib}/medicalreports/medicalreport/${report.reportId}`}>
                        ...
                        <div className={"patient_list_instance"}>
                            ...
                            <div className={"patient_list_buttonbox "}>
                                <Link to={`/pediatrician/patientprofile/${patient.oib}/medicalreports/medicalreport/${medRep.reportId}`}>
                                    <button className={"patient_list_button ped_button_color"}>Otvori</button>
                                </Link>
